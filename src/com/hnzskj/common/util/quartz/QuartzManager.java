...
public class QuartzManager {
	 ...
	 * 默认工作名称后缀job
	 */
	private static String DEFAULT_JOB_NAME_SUFFIX="J";
	 ...
	 * 默认触发器后缀T
	 */
	private static String DEFAULT_TRIGGER_NAME_SUFFIX="T";
	 ...
	 * @version   1.0<br/>
	 */
	public static Scheduler schedulerBuild(String schedulerName,String cronTime,Class<? extends Job> jobClass) {
		try {
			init(schedulerName);
			JobDetail jobDetail = (JobDetail) JobBuilder.newJob(jobClass)
					.withIdentity(DEFAULT_JOB_NAME_SUFFIX, DEFAULT_JOB_GROUP_NAME).build();
			CronTrigger trigger = (CronTrigger) TriggerBuilder
					.newTrigger()
					.withIdentity(DEFAULT_TRIGGER_NAME_SUFFIX, DEFAULT_TRIGGER_GROUP_NAME)
					.withSchedule(
							CronScheduleBuilder.cronSchedule(cronTime))//"0/1 * * * * ?"
					.build();
			if(!scheduler.checkExists(new TriggerKey(DEFAULT_TRIGGER_NAME_SUFFIX, DEFAULT_TRIGGER_GROUP_NAME))){
				scheduler.scheduleJob(jobDetail, trigger);
			}
		} catch (SchedulerException e) {
			e.printStackTrace();
