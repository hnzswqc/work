...
<script type="text/javascript">
	...
	function goNextPage( page ) {
		...
		document.getElementById("limit").value = ${empty pageBean.limit?10:pageBean.limit};
		document.forms.namedItem("queryForm").submit();	
	}

	//分页
	function goLimitPage( limit ) {
		//document.getElementById("reqPage").value = ${empty pageBean.page?1:pageBean.page};
		document.getElementById("reqPage").value = 1;
	...
	<c:forEach begin="${pageBean.beginIndex}" end="${pageBean.endIndex}"
		...
		<c:choose>
			<c:when test="${ pageCount == pageBean.page}">
				<li class="disabled" style="font-weight: bold;">
