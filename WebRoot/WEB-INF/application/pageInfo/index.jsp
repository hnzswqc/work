...
<html>
	<head>
		...
		<title><%=Constant.config_title %>>>图形界面管理</title>
	...
	<body>
		<form action="<%=basePath%>pageInfo/index.shtml" id="queryForm" name="queryForm" method="post">
			<input type="hidden" name="page" id="reqPage" value="${pageBean.page }">
			<input type="hidden" name="limit" id="limit" value="${pageBean.limit }">
			<input type="hidden" name="title"  value="${pageInfo.title }">
			<input type="hidden" name="subTitle"  value="${pageInfo.subTitle }">
			<input type="hidden" name="status"  value="${pageInfo.status }">
		</form>
	<!-- header start -->
	<jsp:include page="/common/header.jsp"></jsp:include>
	<!-- header end -->
	<div class="container-fluid" >
		<div class="panel panel-primary"  style="${(!empty pageInfo.title||!empty pageInfo.subTitle||!empty pageInfo.status)?"":"display: none"}" id="searchPanel">
		   ...
		   <div class="panel-body">
			   <form class="form-inline" id="searchForm" action="<%=basePath%>pageInfo/index.shtml" method="post">
				  <div class="form-group">
				    <label for="id" style="display: inline;">标题：</label>
				    <input type="text"  class="form-control"  id="title" name="title" value="${pageInfo.title }" placeholder="请输入要查询的标题">
				  </div>
				   <div class="form-group">
				    <label for="id" style="display: inline;">副标题：</label>
				    <input type="text"  class="form-control"  id="subTitle" name="subTitle" value="${pageInfo.subTitle }" placeholder="请输入要查询的副标题">
				  </div>
				  <div class="form-group">
				   <label for="name" style="display: inline;">启用状态：</label>
				   <select class="form-control" id="status" name="status" id="status" onchange="javascript:change();">
						  <option value="">请选择</option>
					      <option value="${STATUS_ENABLE }" ${pageInfo.status eq STATUS_ENABLE?'selected':'' }>启用</option>
					      <option value="${STATUS_DISABLE }" ${pageInfo.status eq STATUS_DISABLE?'selected':'' }>停止</option>
					    </select>
				  </div>
		      ...
		      <h3 class="panel-title">
				...
				<thead>
					...
					<c:if test="${!empty pageBean.list}">
						<c:forEach items="${pageBean.list}" var="pageInfo" varStatus="i">
							<tr >
								<td style="text-align: center;">
									<input type="radio" name="uuid" value="${pageInfo.uuid }"/>
								</td>
								<td style="text-align: center;">
									<a href="javascript:update('${pageInfo.uuid }');">${(pageBean.page-1)*pageBean.limit+i.index+1 }</a>
								</td>
								<td>
									 <a href="javascript:showDetail('${pageInfo.uuid }');"class="tooltip-test" data-toggle="tooltip" title="点击标题预览，点击其他进行修改" style="cursor: hand;">${pageInfo.title }</a>
								</td>
								<td>
									<a href="javascript:update('${pageInfo.uuid }');">${pageInfo.subTitle }</a>
								</td>
								<td>
									<a href="javascript:update('${pageInfo.uuid }');">${pageInfo.description }</a>
								</td>
								<td style="text-align: center;">
									<a href="javascript:update('${pageInfo.uuid }');">
									   	<c:choose>
									   		<c:when test="${pageInfo.num eq 1}">显示</c:when>
									   		<c:when test="${pageInfo.num eq 2}"><span style="color:red">隐藏</span></c:when>
									   		<c:otherwise>
									   			未知
									   		</c:otherwise>
									   	</c:choose>
								...
								<td style="text-align: center;">
									<a href="javascript:update('${pageInfo.uuid }');">${pageInfo.status eq STATUS_ENABLE?'启用':'<span style="color:red">未启用<span>' }</a>
								</td>
								<td style="text-align: center;">
									<a href="javascript:update('${pageInfo.uuid }');">${pageInfo.sequenceNo }</a>
								</td>
							</tr>
						</c:forEach>
					</c:if>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="exampleModalLabel">图形界面管理</h4>
      </div>
      <div class="modal-body">
        	<form class="form-horizontal" role="form" id="addForm">
				...
				<div class="form-group">
					 <label for="num" class="col-sm-2 control-label">是否显示标题</label>
					<div class="col-sm-10">
						<select class="form-control" id="num" name="num" id="num">
						   <option value="1" selected="selected">显示</option>
						   <option value="2">隐藏<ption>
					    </select>
					</div>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<select class="form-control" id="status" name="status" id="status">
					      ...
					      <option value="${STATUS_DISABLE }">未启用</option>
					    </select>
					</div>
				</div>
				<div class="form-group">
	...
	<script type="text/javascript">
		...
		function validateForm(){
		    ...
		    $('#addForm').bootstrapValidator({
		            ...
		            $.ajax({
						url:"<%=basePath %>pageInfo/modify.shtml",
						type:'POST',
						data:$form.serialize(),
						dataType:'JSON',
		...
		function update(uuid){
			$.ajax({
				url:"<%=basePath %>pageInfo/findByPrimaryKey.shtml",
				type:'POST',
				data:{"uuid":uuid},
				dataType:'JSON',
				success:function(json){
		...
		function delConfirmClick(){
			...
			Ewin.confirm({message:"<span style='color:red;font-weight:bold;'>确定要删除吗？</span>"}).on(function(e){
				...
				$.ajax({
					url:'<%=basePath%>pageInfo/delete.shtml',
					type:'POST',
					data:{"uuid":uuid},
					dataType:'JSON',
					success:function(json){
		...
		function refreshIconClick(){
			window.location.href="<%=basePath %>pageInfo/index.shtml";
		}
		...
		function showDetail(uuid){
			post("<%=basePath %>pageInfo/show.shtml",{uuid:uuid,type:'show'});
		}
	</script>					    
	</body>
</html>
