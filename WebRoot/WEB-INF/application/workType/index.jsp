...
<html>
	<head>
		...
		<title><%=Constant.config_title %>>>${workType.cateGoryName }</title>
	...
	<body>
		<form action="<%=basePath%>${menu }/index.shtml" id="queryForm" name="queryForm" method="post">
			...
			<input type="hidden" name="cateGory" id="cateGory" value="${workType.cateGory }">
			<input type="hidden" name="cateGoryName" id="cateGoryName" value="${workType.cateGoryName }">
		</form>
	<div class="container-fluid" >
		<div class="panel panel-primary"  style="${!(empty workType.code&&empty workType.text)?"":"display: none"}" id="searchPanel">
		   ...
		   <div class="panel-body">
			   <form class="form-inline" id="searchForm" action="<%=basePath%>${menu }/index.shtml" method="post">
			  	 <input type="hidden" name="cateGory" id="cateGory_searchForm" value="${workType.cateGory }">
				<input type="hidden" name="cateGoryName" id="cateGoryName_searchForm" value="${workType.cateGoryName }">
				  <div class="form-group">
				    <label for="id" style="display: inline;">编码：</label>
				    <input type="text"  class="form-control"  name="code" value="${workType.code }" placeholder="请输入要查询的编码">
				  </div>
				   <div class="form-group">
				    <label for="id" style="display: inline;">名称： </label>
				    <input type="text"  class="form-control"  name="text" value="${workType.text }" placeholder="请输入要查询的名称">
				  </div>
		      ...
		      <h3 class="panel-title">
				...
				<thead>
					...
					<c:if test="${!empty pageBean.list}">
						<c:forEach items="${pageBean.list}" var="wt" varStatus="i">
							<tr >
								<td>
									<input type="radio" name="uuid" value="${wt.uuid }"/>
								</td>
								<td>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="exampleModalLabel">${workType.cateGoryName }</h4>
      </div>
      <div class="modal-body">
        	<form class="form-horizontal" role="form" id="addForm">
        		<input type="hidden" id="uuid" name="uuid"/>
        		<input type="hidden" name="cateGory" id="cateGory_add" value="${workType.cateGory}"/>
				<div class="form-group">
					<label for="modelName" class="col-sm-2 control-label">编码</label>
					<div class="col-sm-10">
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<input type="text" class="form-control" id="text" name="text" placeholder="请输入对应名称" />
					</div>
				</div>
				<div class="form-group">
	...
	<script type="text/javascript">
		...
		function setDefaultValue(){
			$("#cateGory").attr("value","${workType.cateGory}");
			$("#cateGory_add").attr("value","${workType.cateGory}");
			$("#cateGory_searchForm").attr("value","${workType.cateGory}");
			$("#cateGoryName").attr("value","${workType.cateGoryName}");
			$("#cateGoryName_searchForm").attr("value","${workType.cateGoryName}");
